...
from pharma.common.enums import EvalMetric, AnnotationLabel, SegmentationAlgorithm

from pharma.segmentation.segmenter import Segmenter, SegmentationResult
...
class EvaluatorByAnnotation(Evaluator):
    ...
    def __init__(self, annotation: Annotation, homo_mat: Union[np.ndarray, None] = None) -> NoReturn:
        ...
        self.image_mask_ref_text: np.ndarray = pu.to_gray(self.annotation.create_mask_by_labels(
    ...
    def evaluate(self, segmenter: Segmenter) -> NoReturn:
        if config.segmentation.is_alignment_needed() and self.homo_mat is None:
            self.homo_mat = pu.find_homography_matrix(
                pu.to_gray(segmenter.image_not_scaled), pu.to_gray(self.annotation.image_ref)
            )
    ...
    def _evaluate_by_mask(self, image_ver: np.ndarray) -> np.ndarray:
        if config.segmentation.is_alignment_needed():
            image_ver = cv.warpPerspective(
                image_ver, self.homo_mat, pu.swap_dimensions(self.annotation.image_ref.shape)
            )
        ...
        for mask_ref, region_ref in zip(masks_ref, regions_ref):
            ...
            for mask_ver, region_ver in zip(masks_ver, regions_ver):
                if cv.countNonZero(cv.bitwise_and(mask_ref, mask_ver)) > 0:
                    ...
                    pmask_cropped = pu.crop_image_by_contour(mask_ver, contour, True)
                    bmask_cropped = pu.crop_image_by_contour(mask_ref, contour, True)
...
class EvaluatorByVerification(Evaluator):
    ...
    def evaluate(self, segmenter: Segmenter) -> NoReturn:
        if self.homo_mat is None:
            self.homo_mat = pu.find_homography_matrix(
                pu.to_gray(segmenter.image_not_scaled), pu.to_gray(self.image_reference)
            )
    ...
    def _evaluate_by_mask(self, image_ver: np.ndarray, image_ref: np.ndarray):
        image_ver_warped = cv.warpPerspective(
            image_ver, self.homo_mat, pu.swap_dimensions(self.image_reference.shape)
        )
        ...
        for mask_ref, region_ref in zip(masks_ref, regions_ref):
            ...
            for mask_ver, region_ver in zip(masks_ver, polygons_ver):
                if cv.countNonZero(cv.bitwise_and(mask_ref, mask_ver)) > 0:
                    ...
                    mask_ver_cropped = pu.crop_image_by_contour(mask_ver, contour, True)
                    mask_ref_cropped = pu.crop_image_by_contour(mask_ref, contour, True)
