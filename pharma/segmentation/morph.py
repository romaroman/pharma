...
def extract_edges(
    ...
    image_magnitude, image_direction = pu.find_magnitude_and_angle(image_gray)
...
def apply_line_morphology(
    ...
    for line_rotation_angle in line_rotation_angles:
        strel_line_rotated = pu.morph_line(line_length, line_rotation_angle)
    ...
    strel_line_rotated = pu.morph_line(int(line_length / 2), line_rotation_angles[k])
    image_linearly_morphed = cv.dilate(image_bw, strel_line_rotated)
...
def apply_line_morphology_simplified(
    ...
    strel_line_rotated = pu.morph_line(line_length, angle)
    image_linearly_morphed = cv.dilate(image_bw, strel_line_rotated)
...
def clear_borders(image_bw: np.ndarray) -> np.ndarray:
    ...
    contours_to_delete = list(map(lambda p: pu.approximate_contour(p, 0.01), filter(is_contour_invalid, contours)))
