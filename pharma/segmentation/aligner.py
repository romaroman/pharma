...
from pharma.common.enums import AlignmentMethod
...
class Aligner:
    ...
    def align_with_reference(
        ...
        if homo_mat is None:
            homo_mat = pu.find_homography_matrix(pu.to_gray(image_to_warp), pu.to_gray(image_ref))
        return cv.warpPerspective(image_to_warp, homo_mat, pu.swap_dimensions(image_ref.shape))
    ...
    def align_to_corners(cls, image_ver: np.ndarray) -> Tuple[bool, Union[np.ndarray, None]]:
        ...
        while not found_package_border:
            ...
            image_bw = pu.imthresh(image_gray, thresh_adjust=thresh_adjusts.pop())
            ...
            image_filled = pu.fill_holes(image_dilated)
            ...
            image_contour = pu.crop_image_by_contour(image_filled, contour, False)
    ...
    def crop_and_align_contour(cls, image: np.ndarray, contour: np.ndarray) -> np.ndarray:
        ...
        image_bw = pu.imthresh(pu.to_gray(image_cropped), thresh_adjust=-100)
