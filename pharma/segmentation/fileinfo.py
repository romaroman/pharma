...
from pharma.common.enums import FileDatabase
...
class FileInfo(ABC):
    ...
    def get_annotation_pattern(self) -> str:
        return f"*{pu.zfill_n(self.package_class)}_D0{self.distinct}*"

    def get_verification_pattern(self) -> str:
        return f"*{pu.zfill_n(self.package_class)}_D0{self.distinct}_S{pu.zfill_n(self.sample, 3)}*"

    def get_unique_identifier(self) -> str:
        return f"{pu.zfill_n(self.package_class, 4)}_{pu.zfill_n(self.distinct, 2)}_{pu.zfill_n(self.sample, 2)}"
