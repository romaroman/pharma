...
class Morph:
    ...
    @classmethod
    def align_package_to_corners(cls, image_rgb: np.ndarray) -> Union[np.ndarray, None]:
    ...
    @classmethod
    def extract_edges(cls, image_gray: np.ndarray, image_mask: np.ndarray = None,
                      nbins: int = 7, post_morph: bool = False) -> np.ndarray:
    ...
    @classmethod
    def apply_line_morphology_simplified(cls, image_bw: np.ndarray, angle: int, line_length: int) \
            -> Tuple[int, np.ndarray]:
    ...
    @classmethod
    def split_lines(cls, image_bw, axis=0):
        H, W = image_bw.shape
        ...
        horizontal_rectangles = np.asarray([[(0, y1), (W, y2)] for y1, y2 in zip(horizontal_upper, horizontal_lower)], dtype=np.int0)

        return horizontal_rectangles

    @classmethod
    def crop_and_align_contour(cls, image, contour):
