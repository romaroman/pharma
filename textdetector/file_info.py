...
class FileInfoEnrollment(FileInfo):
    ...
    def get_annotation_pattern(
        ...
        return f"PFP_Ph1_P{str(self.package_class).zfill(4)}_D01_S001_C._az360_side1"
    ...
    def to_dict(
        ...
        return dict({
            'angle': self.angle,
            'side': self.side
        }, **super(FileInfoEnrollment, self).to_dict())
...
class FileInfoRecognition(FileInfo):
    ...
    def to_dict(
        ...
        return dict({
            'RS': self.RS,
        }, **super(FileInfoRecognition, self).to_dict())
